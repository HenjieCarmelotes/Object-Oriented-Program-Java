...
public class StringList {
    ...
    public boolean find(String searchItem) {
        ...
        runner = head;  // Start by looking at the head of the list. >>save, get, and record an access to the head (root node).
    ...
    public boolean delete(String deleteItem) {
        ...
        if ( head == null ) {
                // The list is empty, so it certainly doesn't contain string.
            return false;
        }
        else if ( head.item.equals(deleteItem) ) {
                // The string is the first item of the list.  Remove it.
            head = head.next; //assign the next node to the head as if the original head has been removed
            return true;
        }
        else {
            ...
            if ( runner != null && runner.item.equals(deleteItem) ) {
                    ...
                    // Remove it by changing the pointer back in the previous node.
