    ...
    static void towersOfHanoi(int disks, int from, int to, int spare) {
        if (disks == 1) {
            ...
            System.out.printf("Move disk 1 from stack %d to stack %d%n", 
                ...
                // disks on top of it.
            towersOfHanoi(disks-1, from, spare, to); //Suppose that you move 4 discs.
            										//Before executing the next statement after this call,
            										//It has to process the one on top first. Until it 
            										//it reaches 1. So the last value of disk was 2 before 1.
            System.out.printf("Move disk %d from stack %d to stack %d%n",
                                  disks, from, to);
            										//Going back to the 4 discs example, the last value of disk was 2
            										//and subtracting it to 1 yields Move Disk 1 result. 
            										//Notice that the spare value takes the from value and to value takes the to.
