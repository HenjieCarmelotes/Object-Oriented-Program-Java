...
public class ThreadTest1 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numberOfThreads; i++)
            worker[i].start();
        int processorCount = Runtime.getRuntime().availableProcessors(); //count the processor available for JVM.
        System.out.println("Threads have been created and started for this computer with " + processorCount + " processors."); //at this point, the main program thread
        					//has ended. but the threads on top are still running.
        					//after a pause of about 3 seconds, all the threads completed about the same time.
    ...
    private static boolean isPrime(int x) {
        assert x > 1;
        int top = (int)Math.sqrt(x); // Sometimes the endpoint of input data is not known.
        							//use this to notify the program to stop acquiring input in the loop.
        for (int i = 2; i <= top; i++)
            if ( x % i == 0 )
                return false;
        return true;
